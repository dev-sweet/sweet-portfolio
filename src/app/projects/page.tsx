...
export default function ProjectsPage() {
  ...
  if (loading) {
    return (
      <div className="mt-20 min-h-screen bg-[#050609] text-[#F1F5F9] font-sans antialiased">
        <section className="relative container mx-auto lg:px-24 md:px-12 sm:px-8 px-6 pt-16 pb-12 overflow-hidden animate-pulse">
          ...
          <div className="h-10 w-80 bg-[#0D1118] rounded mb-4"></div>
          <div className="h-4 w-96 bg-[#0D1118] rounded"></div>
        </section>
        <section className="container mx-auto lg:px-24 md:px-12 sm:px-8 px-6 mb-16">
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {[1, 2, 3].map((n) => (
              <div
                key={n}
                className="animate-pulse rounded-xl border border-[#1C2633] bg-[#0D1118] h-[380px] flex flex-col overflow-hidden"
              >
                <div className="aspect-video bg-[#080B10] w-full"></div>
                <div className="p-6 flex-1 flex flex-col gap-4">
                  <div className="h-6 w-full bg-[#080B10] rounded"></div>
                  <div className="h-4 w-5/6 bg-[#080B10] rounded"></div>
                  <div className="flex gap-2">
                    <div className="h-4 w-12 bg-[#080B10] rounded"></div>
                    <div className="h-4 w-12 bg-[#080B10] rounded"></div>
                  </div>
                  <div className="mt-auto flex justify-between">
                    <div className="h-4 w-16 bg-[#080B10] rounded"></div>
                    <div className="h-4 w-20 bg-[#080B10] rounded"></div>
                  </div>
                </div>
              </div>
            ))}
  ...
  return (
    <div className="min-h-screen text-[#F1F5F9] font-sans antialiased">
      <style>{`
        ...
        .group:hover .group-accent-border { border-color: #29384A; }
      ...
      <section className="relative container mx-auto lg:px-24 md:px-12 sm:px-8 px-6 pt-16 pb-12 overflow-hidden">
        ...
        <div className="relative">
          ...
          <p className="text-[#A1ACBA] max-w-xl leading-relaxed">
            A mix of full-stack apps, APIs, and weekend experiments — mostly
            Next.js, Node, and whatever new library I wanted an excuse to try.
          </p>
        </div>
      </section>
      ...
      {featuredProject && (
        <section className="container mx-auto lg:px-24 md:px-12 sm:px-8 px-6 mb-8">
          <div className="group rounded-xl border border-[#1C2633] bg-[#0D1118] hover-border-accent transition-all overflow-hidden">
            <div className="grid md:grid-cols-2">
              <Link
                href={`/projects/${featuredProject.id}`}
                className="relative min-h-[240px] overflow-hidden block"
              >
                <img
                ...
                />
                <span className="absolute top-4 left-4 flex items-center gap-2 px-2.5 py-1 rounded-md bg-[#080B10]/80 border border-[#1C2633] font-mono text-xs text-[#F1F5F9]">
              ...
              <div className="p-8 flex flex-col justify-center">
                ...
                <p className="text-[#A1ACBA] mb-5 leading-relaxed">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {featuredProject.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-2.5 py-1 rounded-md text-xs font-mono border border-[#1C2633] text-[#A1ACBA]"
                    >
                      {tag}
                    </span>
                  ))}
                ...
                <div className="flex items-center gap-4 text-sm font-mono">
                  ...
                  <a
                    href={
                      featuredProject.github || "https://github.com/dev-sweet"
                    }
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-[#1C2633] text-[#A1ACBA] hover:text-[#F1F5F9] hover:border-[#29384A] transition-colors"
                  >
                    <Github className="w-4 h-4" />
                    source
                  </a>
                  <span className="ml-auto text-[#667386]">
        ...
        {regularProjects.length > 0 && (
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {regularProjects.map((project) => (
              <div
                key={project.id}
                className="group flex flex-col rounded-xl border border-[#1C2633] bg-[#0D1118] group-accent-border hover:-translate-y-1 transition-all overflow-hidden"
              >
                <Link
                ...
                >
                  ...
                  />
                  <span className="absolute top-3 left-3 flex items-center gap-1.5 px-2 py-0.5 rounded bg-[#080B10]/80 border border-[#1C2633] font-mono text-[11px] text-[#F1F5F9]">
                    <span className="inline-flex rounded-full h-1.5 w-1.5 bg-[#22C55E]"></span>
                    live
                  </span>
                </Link>
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <p className="text-sm text-[#A1ACBA] leading-relaxed mb-4 line-clamp-2">
                    {project.description}
                  </p>
                  <div className="flex flex-wrap gap-1.5 mb-5">
                    {project.tags.map((tag) => (
                      <span
                        key={tag}
                        className="px-2 py-0.5 rounded text-[11px] font-mono border border-[#1C2633] text-[#667386]"
                      >
                        {tag}
                      </span>
                    ))}
                  ...
                  <div className="mt-auto flex items-center justify-between text-xs font-mono text-[#667386]">
                    ...
                    <span className="flex items-center gap-3">
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="group-accent-text hover:text-[#F1F5F9] transition-colors"
                      >
                        <ExternalLink className="w-4 h-4" />
                      </a>
                    </span>
